...
import { ShieldCheck, Zap, Award, Info, Droplets, Hammer } from 'lucide-react';

const BATH_VOLUMES = [
  { zone: 'Volumen 0', desc: 'Interior de la bañera o plato de ducha.', ip: 'IPX7', notes: 'Solo MBTS ≤12V. Prohibidos mecanismos.' },
  { zone: 'Volumen 1', desc: 'Superficie vertical hasta 2.25m sobre el volumen 0.', ip: 'IPX4 (IPX5 si duchas con chorro a presión)', notes: 'Solo aparatos MBTS o calentadores fijos Clase I con toma de tierra.' },
  { zone: 'Volumen 2', desc: '0.60m horizontal desde el volumen 1.', ip: 'IPX4 (IPX5 en locales con duchas a presión)', notes: 'Bases de enchufe no permitidas salvo MBTS o alimentadas por transformador de aislamiento.' },
  { zone: 'Volumen 3 / Fuera de volúmenes', desc: 'Resto del cuarto de baño.', ip: 'IPX4 recomendado (según proyecto)', notes: 'Bases de enchufe permitidas con diferencial de 30mA.' }
];
    ...
    <div className="p-8 md:p-12 space-y-16 bg-slate-900 min-h-full tab-content-enter">
      ...
      <section className="max-w-5xl mx-auto space-y-8">
        <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
          ...
          <h3 className="text-xl font-bold text-white uppercase tracking-wider">Volúmenes de Baño (ITC-BT-27)</h3>
        </div>
        <p className="text-slate-400 text-xs max-w-2xl">Crítico para seguridad: cada volumen exige un grado de protección IPX mínimo y restringe qué mecanismos pueden instalarse por el riesgo de contacto con agua.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {BATH_VOLUMES.map(v => (
            <div key={v.zone} className="bg-slate-950 border border-slate-800 rounded-[24px] p-6">
              <div className="flex justify-between items-start mb-3">
                <h4 className="text-white font-black text-sm uppercase">{v.zone}</h4>
                <span className="bg-blue-500/20 text-blue-400 text-[10px] font-black px-3 py-1 rounded-full border border-blue-500/30">{v.ip}</span>
              </div>
              <p className="text-[11px] text-slate-400 mb-2">{v.desc}</p>
              <p className="text-[10px] text-slate-500 italic border-l-2 border-orange-500/40 pl-3">{v.notes}</p>
            </div>
          ))}
        </div>
      </section>
