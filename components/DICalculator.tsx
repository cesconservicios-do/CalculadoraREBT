...
import { CABLE_SECTIONS, CONDUCTIVITY, NORMALIZED_POWERS_MONO, NORMALIZED_POWERS_TRI, STORAGE_KEYS } from '../constants';
import { CalculationResult, LocationType, PhaseType, ConductorMaterial, InsulationType, HistoryEntry } from '../types';
import { Calculator, CheckCircle2, AlertTriangle, Cpu, Zap, Thermometer, Layers, Keyboard, Info, History, X } from 'lucide-react';

const loadHistory = (): HistoryEntry[] => {
  try {
    const raw = localStorage.getItem(STORAGE_KEYS.DI_HISTORY);
...
const DICalculator: React.FC = () => {
  ...
  useEffect(() => {
    const handle = setTimeout(() => {
      ...
      if (last && last.power === power && last.length === length && last.material === material && last.phase === phase) {
        return;
      }
      ...
      localStorage.setItem(STORAGE_KEYS.DI_HISTORY, JSON.stringify(updated));
      setHistory(updated);
    }, 800);
    return () => clearTimeout(handle);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [power, length, material, phase, result.section]);
    ...
    <div className="p-6 md:p-10 bg-slate-900 min-h-full tab-content-enter">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-700 pb-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            aria-label="Ver historial de cálculos"
            className="flex items-center gap-1.5 text-[10px] font-black bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 px-3 py-1 rounded-full uppercase tracking-widest transition-all"
          >
      ...
      {showHistory && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-sm bg-slate-950 border-l border-slate-800 h-full p-6 overflow-y-auto shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-white font-black uppercase text-sm tracking-widest flex items-center gap-2"><History size={16} className="text-orange-500" /> Últimos cálculos</h3>
              <button onClick={() => setShowHistory(false)} aria-label="Cerrar historial" className="text-slate-500 hover:text-white">
                <X size={20} />
              </button>
            </div>
            {history.length === 0 ? (
              <p className="text-slate-600 text-xs italic">Aún no hay cálculos guardados.</p>
            ) : (
              <div className="space-y-3">
                {history.map(entry => (
                  <button
                    ...
                    className="w-full text-left bg-slate-900 border border-slate-800 hover:border-orange-500/50 rounded-2xl p-4 transition-all"
                  >
                    <div className="text-[9px] text-slate-500 font-mono-tech mb-2">{entry.date}</div>
                    <div className="flex justify-between items-center">
                      <div className="text-xs text-slate-300">
                        <span className="font-bold">{(entry.power / 1000).toFixed(2)}kW</span> · {entry.length}m · {entry.material} · {entry.phase === 'monophasic' ? 'Mono' : 'Tri'}
                      </div>
                      <div className="text-orange-500 font-mono-tech font-black text-sm">{entry.section}mm²</div>
                    </div>
                  </button>
                ))}
              </div>
